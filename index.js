...
const startAtlas = async () => {
  ...
  // Request pairing code once the socket has had time to finish its
  // handshake with WhatsApp's servers. Calling this immediately after
  // makeWASocket() causes "Connection Closed / 428 Precondition Required"
  // because the underlying WebSocket isn't open yet.
  if (!Atlas.authState.creds.registered) {
    ...
    setTimeout(async () => {
      try {
        ...
        console.log(chalk.green(`🔑 ATLAS PAIRING CODE: ${code}`));
      } catch (err) {
        console.error(
          chalk.redBright(`[ ATLAS ] Pairing code request failed: ${err.message}`),
        );
      }
    }, 3000);
  }

  AtlasSocket = Atlas; // expose for pairing
